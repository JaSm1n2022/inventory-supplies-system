import { Accordion, AccordionDetails, AccordionSummary, Avatar, Button, CircularProgress, Divider, FormControlLabel, Grid, Paper, Tooltip, Typography } from "@mui/material";
...
const Orders = (props) => {
    ...
    useEffect(() => {
        const dateRange = Helper.formatDateRangeByCriteriaV2('last7Days', 'created');
                ...
                {orderList.map(item => {
                    return (
                        ...
                        <Accordion >
                            ...
                            >
                                <Grid justifyContent="space-between" container>
                                    <div style={{ display: 'inline-flex', gap: 4 }}>
                                        <Typography variant="body2" >{moment(item.created_at).format('MM/DD/YY')}</Typography>
                                        <Typography variant="body2" style={{ fontWeight: 'bold' }}>{item.patient_name || ''}</Typography>

                                    </div>
                                    <Typography variant="body2" style={{ fontWeight: 'bold' }}>{item.status}</Typography>
                                </Grid>
                            </AccordionSummary>
                            <AccordionDetails>
                                ...
                                {item.items.map((i, indx) => {
                                    return (
                                        <Grid container direction="row" spacing={1}>
                                            ...
                                            <Grid item xs={4}>
                                                <Typography>{i.qty} {i.uom || ''}</Typography>
                                            </Grid>
                                        </Grid>
                                    )
                                })}
                            ...
                            <div style={{ display: 'inline-flex', gap: 4, paddingBottom: 2, paddingLeft: 4 }}>
                                <Button variant="contained" size="small" color="primary">Re-Order</Button>
                                {item.status === 'Order' &&
