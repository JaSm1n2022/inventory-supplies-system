...
const Supply = (props) => {
  ...
  const placeHolderHandler = () => {
    ...
    for (const it of items) {
      ...
      for (const fld of it.fields) {
        ...
        if (
          fld.isMandatory &&
          fld.component === "select" &&
          (!fld.value || (fld.value && !fld.value.name))
        ) {
          fld.error = {
            isError: true,
            errorMsg: "Required Field",
        ...
        if (
          fld.isMandatory &&
          fld.component === "textfield" &&
          (!fld.value ||
            (fld.type === "number" && parseInt(fld.value, 10) <= 0))
        ) {
          fld.error = {
            isError: true,
            errorMsg: "Required Field",
    ...
    if (isWithError) {
      ...
      isLoading = true;
      const payload = {
        reference: `DCH-${patient.id}-${moment(new Date()).format(
          "YYYYMMDDHHmm"
        )}`,
  ...
  if (
    ...
    props.createOrderState.status === ACTION_STATUSES.SUCCEED
  ) {
    isLoading = false;
  ...
  return (
    <React.Fragment>
      {isLoading ? (
        <div>
          <CircularProgress></CircularProgress>Saving...
        </div>
      ) : (
        <Grid
          container
                          ...
                          return (
                            <React.Fragment key={`field${index}`}>
                              {field.component === "select" ? (
                                <Grid
                                  item
                                  xs={field.colspan ? field.colspan : 6}
                                >
                                  <SingleWithClearAutoComplete
                                    {...field}
                                    source={field}
                                    isError={
                                      field.error ? field.error.isError : false
                                    }
                                    errorMsg={
                                      field.error ? field.error.errorMsg : ""
                                    }
                                    value={field.value || DEFAULT_ITEM}
                                    onSelectHandler={
                                      autoCompleteDetailInputHander
                                    }
                                    onChangeHandler={onChangeDetailInputHandler}
                                  />
                                </Grid>
                                ...
                                >
                                  <RegularTextField
                                    ...
                                    size={12}
                                    value={field.value || ""}
                                    onChange={onChangeDetailInputHandler}
                                  />
                                </Grid>
                              )}
                            </React.Fragment>
