...
import {
  distributionCreateStateSelector,
  distributionDeleteStateSelector,
...
import {
  ...
  attemptToDeleteTemplate,
  attemptToFetchTemplate,
  attemptToUpdateTemplate,
  resetCreateTemplateState,
  resetDeleteTemplateState,
  resetFetchTemplateState,
  resetUpdateTemplateState,
} from "../../../store/actions/templateAction";
import {
  templateCreateStateSelector,
  templateDeleteStateSelector,
  templateListStateSelector,
  templateUpdateStateSelector,
...
const Distribution = (props) => {
  ...
  const [isDistributionsCollection, setIsDistributionsCollection] =
    useState(true);
  const [isCreateDistributionCollection, setIsCreateDistributionCollection] =
    useState(true);
  const [isUpdateDistributionCollection, setIsUpdateDistributionCollection] =
    useState(true);
  const [isDeleteDistributionCollection, setIsDeleteDistributionCollection] =
    useState(true);
  const [generalForm, setGeneralForm] = useState(undefined);
  const [detailForm, setDetailForm] = useState([]);
  const [isCreateTemplateCollection, setIsCreateTemplateCollection] =
    useState(true);
  const [isUpdateTemplateCollection, setIsUpdateTemplateCollection] =
    useState(true);
  const [isDeleteTemplateCollection, setIsDeleteTemplateCollection] =
    useState(true);
  ...
  if (
    ...
    TOAST.ok("Distribution updated successful.");
    props.updateStock(forStockUpdates);
    props.listDistributions({ from: dateFrom, to: dateTo });
  }
  console.log(
  ...
  if (
    ...
    TOAST.ok("Distribution deletion successful.");
    props.updateStock(forStockUpdates);
      ...
      const found = temp.filter(
        (data) =>
          data.description.toLowerCase().indexOf(keyword.toLowerCase()) !==
            -1 ||
          data.patient_name.toLowerCase().indexOf(keyword.toLowerCase()) !==
            -1 ||
