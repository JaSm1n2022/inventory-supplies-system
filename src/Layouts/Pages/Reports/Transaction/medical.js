...
const MedicalExpensesReport = (props) => {
    ...
    useEffect(() => {
        if (!isProcessCollection && props.transactions.status === ACTION_STATUSES.SUCCEED) {
            ...
            const record = [...props.transactions.data].filter(p => p.category.toLowerCase() !== 'office');
    ...
    return (
        <React.Fragment>
            <Grid justifyContent="space-between" container style={{ padding: 10 }}>
                <Typography variant="h5">Incontinence/Medical Purchased Report</Typography>
            ...
            <Grid justifyContent="space-between" container style={{ padding: 10 }}>
                <Typography variant="body1" color="textSecondary">*** This report is based on reported product that is related to Incontinence/Medical ***</Typography>
            </Grid>
...
export default connect(mapStateToProps, mapDispatchToProps)(MedicalExpensesReport);
