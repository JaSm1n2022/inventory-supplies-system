...
import {
  attemptToFetchTransaction,
  resetFetchTransactionState,
...
import {
  attemptToFetchProduct,
  resetFetchProductState,
} from "../../store/actions/productAction";
import {
  attemptToFetchStock,
  resetFetchStockState,
...
const Dashboard = (props) => {
  ...
  const plotHandlerOriginal = (source) => {
    ...
    for (const plot of plots) {
      if (plot.productId) {
        ...
        if (currentStock) {
          plot.currentStock = tempStock.find(
            (stk) => stk.productId === plot.productId
          ).qty_on_hand;
          if (currentStock.qty_on_hand === 0) {
            ...
            const diff1 =
              parseInt(currentStock.qty_on_hand, 10) - parseInt(plot.threshold);
    ...
    uIds.forEach((u) => {
      ...
      if (orders > 0) {
        ...
        const cartonCnt =
          Math.ceil(parseFloat(orders / sel[0].cnt)) === 0
            ? 1
            : Math.ceil(parseFloat(orders / sel[0].cnt));
        estimatedSupplyGrandTotal[source] =
          parseFloat(estimatedSupplyGrandTotal[source]) +
          parseInt(cartonCnt) * sel[0].unitPrice;
  ...
  if (
    ...
    for (const patient of patientList) {
      ...
      const underwears = supplies.filter(
        (supply) => supply.category === "Underwear/Pull-ups"
      );
      const underpads = supplies.filter(
        (supply) => supply.category === "Underpads"
      );
  ...
  if (
    ...
    transactionDashboard.series = [
      parseFloat(officeAmount),
    ...
    providerDashboard.other =
      grandTotal - amazonAmount - medlineAmount - mckessonAmount;

    providerDashboard.series = [
      parseFloat(amazonAmount),
      parseFloat(medlineAmount),
      parseFloat(mckessonAmount),
      parseFloat(providerDashboard.other),
    ];
        ...
        <React.Fragment>
          ...
          <div style={{ width: 500, paddingTop: 20 }}>
            <DateTypeAutoComplete
              ...
              name="dateType"
              placeholder={
                dateSelected.name ? `Date : ${dateFrom} to ${dateTo}` : "Date"
              }
          ...
          >
            ...
            <Tab
              value="two"
              style={{ fontSize: 14 }}
              label="SUPPLIES TRANSACTION PAYMENTS"
              {...a11yProps("two")}
            />
            <Tab
              value="three"
              style={{ fontSize: 14 }}
              label="PAYMENT METHOD TRACKING"
              {...a11yProps("three")}
            />
            <Tab
              value="four"
              style={{ fontSize: 14 }}
              label="ORDER PLOT STRATEGY"
              {...a11yProps("four")}
            />
          </Tabs>
          ...
          <TabPanel value={value} index="three">
            <PrintReport
              source={"cardHistoryReport"}
              details={cardTransaction}
              dateFrom={dateFrom}
              dateTo={dateTo}
            />
          </TabPanel>
          <TabPanel value={value} index="four">
            <FormControl component="fieldset">
              <FormLabel component="legend">
                ** Plot View based on patient existing delivery supplies record
            ...
            {plotView === "brief" ? (
              <SupplyPlot
                title={"BRIEF"}
                patientPlot={patientSupplyPlot.brief}
                estimatedGrandTotal={estimatedSupplyGrandTotal.brief}
                unusedSummary={unusedPlotSummary.brief}
                summary={plotSummary.brief}
              />
            ) : plotView === "underpad" ? (
              <SupplyPlot
                title={"UNDERPAD"}
                patientPlot={patientSupplyPlot.underpad}
                estimatedGrandTotal={estimatedSupplyGrandTotal.underpad}
                unusedSummary={unusedPlotSummary.underpad}
                summary={plotSummary.underpad}
              />
            ) : plotView === "underwear" ? (
              <SupplyPlot
                title={"UNDERWEAR"}
                patientPlot={patientSupplyPlot.underwear}
                estimatedGrandTotal={estimatedSupplyGrandTotal.underwear}
                unusedSummary={unusedPlotSummary.underwear}
                summary={plotSummary.underwear}
              />
            ) : plotView === "wipe" ? (
              <SupplyPlot
                title={"WIPE"}
                patientPlot={patientSupplyPlot.wipe}
                estimatedGrandTotal={estimatedSupplyGrandTotal.wipe}
                unusedSummary={unusedPlotSummary.wipe}
                summary={plotSummary.wipe}
              />
            ) : plotView === "glove" ? (
              <SupplyPlot
                title={"GLOVE"}
                patientPlot={patientSupplyPlot.glove}
                estimatedGrandTotal={estimatedSupplyGrandTotal.glove}
                unusedSummary={unusedPlotSummary.glove}
                summary={plotSummary.glove}
              />
            ) : plotView === "ensureVanilla" ? (
              <SupplyPlot
                title={"ENSURE VANILLA"}
                patientPlot={patientSupplyPlot.ensureVanilla}
                estimatedGrandTotal={estimatedSupplyGrandTotal.ensureVanilla}
                unusedSummary={unusedPlotSummary.ensureVanilla}
                summary={plotSummary.ensureVanilla}
              />
            ) : plotView === "ensureChocolate" ? (
              <SupplyPlot
                title={"ENSURE CHOCOLATE"}
                patientPlot={patientSupplyPlot.ensureChocolate}
                estimatedGrandTotal={estimatedSupplyGrandTotal.ensureChocolate}
                unusedSummary={unusedPlotSummary.ensureChocolate}
                summary={plotSummary.ensureChocolate}
              />
            ) : plotView === "ensureStrawberry" ? (
              <SupplyPlot
                title={"ENSURE STRAWBERRY"}
                patientPlot={patientSupplyPlot.ensureStrawberry}
                estimatedGrandTotal={estimatedSupplyGrandTotal.ensureStrawberry}
                unusedSummary={unusedPlotSummary.ensureStrawberry}
                summary={plotSummary.ensureStrawberry}
              />
            ) : null}
          </TabPanel>
        </React.Fragment>
